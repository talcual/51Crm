<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        <x-input-label for="pipeline_stage_id" :value="__('Pipeline Stage')" />
    ...
    <div>
        ...
        <select id="status" name="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            @foreach(['open' => __('Open'), 'won' => __('Won'), 'lost' => __('Lost')] as $value => $label)
    ...
    @if(isset($deal))
        <div>
            <x-input-label for="closed_date" :value="__('Closed Date')" />
