            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="GET" action="{{ route('clients.index') }}" class="mb-4">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search by name, email or company...') }}"
                            class="border-gray-300 rounded-md shadow-sm w-full md:w-1/3 focus:ring-blue-500 focus:border-blue-500">
                    </form>
                                ...
                                @forelse($clients as $client)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $client->company ?? __('N/A') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                                @if($client->type === 'company') bg-blue-100 text-blue-800
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ $client->assignedUser->name ?? __('Unassigned') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('clients.show', $client) }}" class="text-blue-600 hover:text-blue-900 mr-3">{{ __('View') }}</a>
                                            <a href="{{ route('clients.edit', $client) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('Edit') }}</a>
                                            <form action="{{ route('clients.destroy', $client) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('{{ __('Are you sure you want to delete this client?') }}')">{{ __('Delete') }}</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                            {{ __('No clients found.') }} <a href="{{ route('clients.create') }}" class="text-blue-600 hover:text-blue-900">{{ __('Create your first client') }}</a>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
