    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="p-6">
                    ...
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <dd class="text-sm text-gray-900">{{ $client->company ?? 'N/A' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Type</dt>
                            <dd class="text-sm text-gray-900">{{ ucfirst($client->type) }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Assigned To</dt>
                            <dd class="text-sm text-gray-900">{{ $client->assignedUser->name ?? 'Unassigned' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Address</dt>
                            <dd class="text-sm text-gray-900">{{ $client->address ?? 'N/A' }}, {{ $client->city ?? '' }} {{ $client->state ?? '' }} {{ $client->postal_code ?? '' }} {{ $client->country ?? '' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Originated From Lead</dt>
                            <dd class="text-sm text-gray-900">{{ $client->lead->name ?? 'N/A' }}</dd>
                        </div>
                        @if($client->notes)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Deals ({{ $client->deals->count() }})</h3>
                        <ul class="space-y-2">
                            ...
                            @empty
                                <li class="text-sm text-gray-500">No deals yet.</li>
                            @endforelse
                        </ul>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Quotes ({{ $client->quotes->count() }})</h3>
                        <ul class="space-y-2">
                            ...
                            @empty
                                <li class="text-sm text-gray-500">No quotes yet.</li>
                            @endforelse
                        </ul>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Payments ({{ $client->payments->count() }})</h3>
                        <ul class="space-y-2">
